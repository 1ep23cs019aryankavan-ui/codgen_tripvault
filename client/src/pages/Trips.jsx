...
export default function Trips() {
  ...
  const handleCreate = async (tripData, photoFiles) => {
    try {
      ...
      // Upload photos via the Cloudinary /upload endpoint (one file per
      // request — field name must be "image"). The first photo auto-becomes
      // the trip's coverImage.
      if (photoFiles && photoFiles.length > 0) {
        for (const file of photoFiles) {
          const formData = new FormData()
          formData.append('image', file)
          await axios.post(`/api/trips/${data.trip._id}/upload`, formData, {
            headers: {
          ...
          <div className="space-y-4">
            {trips.map((trip) => (
              <Link
                key={trip._id}
                to={`/trips/${trip._id}`}
                className="block overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm transition hover:border-primary-300 hover:shadow-md"
              >
                {/* Cover image (Week 3) */}
                {trip.coverImage && (
                  <img
