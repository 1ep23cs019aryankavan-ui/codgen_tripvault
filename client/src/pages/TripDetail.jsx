...
export default function TripDetail() {
  ...
  const fetchTrip = async () => {
    ...
    try {
      const { data } = await axios.get(`/api/trips/${id}`, { headers })
      setTrip(data.trip)
  ...
  const handleUpdate = async (tripData, photoFiles) => {
    ...
    try {
      const { data } = await axios.put(`/api/trips/${id}`, tripData, { headers })
  ...
  const handleDeletePhoto = async (photoId) => {
    ...
    try {
      await axios.delete(`/api/trips/${id}/photos/${photoId}`, { headers })
      ...
      <main className="mx-auto w-full max-w-4xl px-4 py-8 sm:px-6">
            ...
            <div className="flex-1">
              ...
              {trip.rating > 0 && (
                <div className="mt-3 flex items-center gap-2">
                  ...
                  <span className="text-sm font-medium text-gray-600">{trip.rating} / 5</span>
                </div>
              )}
            </div>
            {trip.isOwner && (
        ...
        {showEdit && trip.isOwner && (
          <div className="mb-6 rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
            ...
            <TripForm initialTrip={trip} onSubmit={handleUpdate} submitLabel="Save changes" />
          </div>
        )}
