...
export default function TripDetail() {
  ...
  const [legacyPhotos, setLegacyPhotos] = useState([]) // old Buffer-based photos
  ...
  // Upload photos via the Cloudinary /upload endpoint (one file per request)
  const uploadPhotos = async (tripId, files) => {
    const token = localStorage.getItem('token')
    for (const file of files) {
      const formData = new FormData()
      formData.append('image', file)
      await axios.post(`/api/trips/${tripId}/upload`, formData, {
        headers: {
  ...
  const handleUpdate = async (tripData, photoFiles) => {
    try {
      const token = localStorage.getItem('token')
      ...
      if (photoFiles && photoFiles.length > 0) {
        await uploadPhotos(id, photoFiles)
      }

      setTrip(data.trip)
      setShowEdit(false)
      fetchTrip() // refresh to show newly uploaded photos
    } catch (err) {
  ...
  const handleUpload = async (e) => {
    ...
    setUploading(true)
    try {
      await uploadPhotos(id, selectedFiles)
      setSelectedFiles([])
      fetchTrip()
    } catch (err) {
  ...
  // Delete a URL-based photo by index (Week 3 — singular "photo" path)
  const handleDeletePhoto = async (index) => {
    if (!confirm('Delete this photo?')) return
    try {
      const token = localStorage.getItem('token')
      await axios.delete(`/api/trips/${id}/photo/${index}`, {
        headers: { Authorization: `Bearer ${token}` },
      })
      fetchTrip() // refresh
    } catch (err) {
  ...
  // Combine new URL photos and old Buffer photos for display
  const urlPhotos = trip.photos || []
  const totalPhotos = urlPhotos.length + legacyPhotos.length
          ...
          <div className="flex items-start justify-between">
            <div className="flex-1">
              ...
              <div className="mt-2 flex flex-wrap items-center gap-4 text-sm text-gray-500">
                ...
                <span className="flex items-center gap-1.5">
                  <CameraIcon /> {totalPhotos} photo{totalPhotos !== 1 ? 's' : ''}
                </span>
              </div>
              {trip.rating > 0 && (
        ...
        <div>
          ...
          {totalPhotos === 0 ? (
            ...
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
              {/* New URL-based photos (Week 3) — delete by index */}
              ...
              {/* Legacy Buffer-based photos (display only) */}
